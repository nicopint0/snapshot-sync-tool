...
const PRICE_IDS: Record<string, string> = {
  individual: "price_1Sw9GzEJFwq29nQFZAU9VZ7a",
  profesional: "price_1Sw9MYEJFwq29nQFZozwX19e",
  business: "price_1Sw9MkEJFwq29nQFXTLtfsCo",
};
...
serve(async (req) => {
  ...
  try {
    ...
    // Determine price ID - either passed directly or by plan name
    const selectedPriceId = priceId || PRICE_IDS[planId];
    if (!selectedPriceId) {
      throw new Error("Invalid plan or price ID");
    }
    logStep("Selected price", { selectedPriceId });
